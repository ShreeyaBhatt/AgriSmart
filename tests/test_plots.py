...
def _stub_soil(monkeypatch):
    ...
    monkeypatch.setattr("app.backend.routers.plots.build_soil_profile", fake_build)


async def test_create_and_list_plot_with_soil(auth_client):
    client, headers, _ = auth_client
    r = await client.post("/api/plots", headers=headers,
                          json={"name": "North field", "lat": 22.31, "lon": 73.18, "area_ha": 1.5})
...
async def test_timeline_merges_activity(auth_client):
    client, headers, _ = auth_client
    plot_id = (await client.post("/api/plots", headers=headers,
               json={"name": "P", "lat": 1.0, "lon": 2.0})).json()["id"]

    assert (await client.post("/api/irrigation", headers=headers,
            json={"plot_id": plot_id, "amount_mm": 20, "note": "furrow"})).status_code == 201
    assert (await client.post("/api/actions", headers=headers,
            json={"plot_id": plot_id, "action_type": "spray", "details": "neem"})).status_code == 201

    tl = await client.get(f"/api/plots/{plot_id}/timeline", headers=headers)
...
async def test_farmers_cannot_see_each_others_plots(client):
    ...
    a_plot = (await client.post("/api/plots", headers=a,
              json={"name": "A field", "lat": 1, "lon": 1})).json()["id"]

    assert (await client.get("/api/plots", headers=b)).json() == []
    assert (await client.get(f"/api/plots/{a_plot}", headers=b)).status_code == 404
    assert (await client.delete(f"/api/plots/{a_plot}", headers=b)).status_code == 404
    assert (await client.get(f"/api/plots/{a_plot}", headers=a)).status_code == 200
