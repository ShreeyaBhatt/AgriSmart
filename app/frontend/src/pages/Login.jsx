  ...
  return (
    <div className="login-hero relative hidden overflow-hidden px-10 py-12 text-white md:flex md:w-[42%] md:flex-col md:justify-center">
      ...
      <div className="relative">
        ...
        <h1 className="mt-6 text-2xl font-bold tracking-tight">{t("login.heroTitle")}</h1>
        <p className="mt-2 text-sm text-brand-100">{t("login.heroTagline")}</p>
        <ul className="mt-8 space-y-3">
          {HERO_BULLETS.map((b) => (
            <li key={b.key} className="flex items-center gap-2.5 text-sm text-brand-50">
...
function OtpStep({ t, phone, demoOtp, otp, setOtp, busy, error, onVerify, onChangeNumber, onResend }) {
  return (
    <div key="otp" className="animate-fade-up space-y-4">
      <h2 className="text-base font-semibold text-ink">{t("login.otpTitle")}</h2>
      <p className="text-xs text-muted">
        {t("login.otpSubtitle")} <span className="font-medium text-ink">{phone}</span>
      </p>
      ...
      <div className="flex items-center justify-between text-xs">
        <button type="button" onClick={onChangeNumber} className="font-medium text-muted hover:text-ink">
          {t("login.changeNumber")}
        </button>
        <button type="button" onClick={onResend} className="font-medium text-brand-700 hover:text-brand-800">
          {t("login.resend")}
        </button>
      </div>
    </div>
        ...
        <div>
          ...
          {form.crop === t("login.cropOther") && (
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, cropOther: e.target.value }))}
              required
            />
          )}
        </div>
