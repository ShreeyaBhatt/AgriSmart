...
def _formula_tips(
    water_over: float, chem_over: float, crop_health: float,
) -> list[str]:
...
async def compute_score(req: SustainabilityRequest) -> SustainabilityScore:
    ...
    # Try AI validation
    ai_tips, ai_notes = await _ai_validate(req, score, band, water_over, chem_over, crop_health)
    if ai_tips:
        tips = ai_tips
        ai_validated = True
    else:
        tips = _formula_tips(water_over, chem_over, float(crop_health))
        ai_validated = False
