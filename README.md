...
- **Farm Assistant** — a grounded RAG pipeline over a disease-card knowledge base plus the farmer's own plot data, answering in English, Hindi, Gujarati, Marathi, Tamil, Telugu, or Punjabi, with voice input and output. Runs on Gemini when a key is configured, or fully offline otherwise. Voice input is transcribed locally by this backend (faster-whisper) rather than a cloud speech API, so it keeps working with no internet beyond reaching your own server.
